...
public class AddEditTravelReq {
    ...
    private RichSelectBooleanCheckbox consent;
    private RichSelectOneChoice travelCode;
...
//    }
    ...
    public void valChangeTravelPurpose(ValueChangeEvent valueChangeEvent) {
        ...
        System.out.println("travelPurposeBinding----------------------T----------------------- " + travelPurposeBinding.toString());
     //   DCBindingContainer bindings = ADFUtils.getDCBindingContainer();
     //   AttributeBinding personIdBinding = (AttributeBinding) bindings.getControlBinding("PersonId");
     ...
     String reqPurpose ;
        ...
        reqPurpose = (String) ADFContext.getCurrent()
                                   .getSessionScope()
                                   .get("reqPurpose");
        System.out.println("Req Purpose--- " + reqPurpose);
        if (reqPurpose != null && reqPurpose.equals("others")){
        
            String personNumnber =
                HdrVo.getCurrentRow().getAttribute("Trans_EmpNo") == null ? " " :
                HdrVo.getCurrentRow()
                                                                                                             .getAttribute("Trans_EmpNo")
                                                                                                             .toString();    
            System.out.println("personNumnber "+ personNumnber);
            
            ViewObject empViewVO =
                ADFUtils.getApplicationModuleForDataControl("Oando_AMDataControl").findViewObject("Employee_View_ROVO");
            ViewCriteria viewCriteria = empViewVO.createViewCriteria();
            ViewCriteriaRow viewCriteriaRow = viewCriteria.createViewCriteriaRow();
            viewCriteriaRow.setAttribute("PersonNumber", personNumnber);
            viewCriteria.addRow(viewCriteriaRow);
            empViewVO.applyViewCriteria(viewCriteria);
            empViewVO.executeQuery();
            if (empViewVO.getEstimatedRowCount() > 0) {
                Row row = empViewVO.first();
                row.getAttribute("PersonId");
                Object personVal = row.getAttribute("PersonId");
                System.out.println("personVal --- "+ personVal);
                ADFContext.getCurrent()
                          .getSessionScope()
                          .put("personId", personVal);
            }
            b = (BigDecimal) ADFContext.getCurrent()
                                          .getSessionScope()
                                          .get("personId");  
            
            System.out.println(" Person Id is " + b);
     ...
     System.out.println("b value is --" + b);
        ...
        if (personId != null && !"".equalsIgnoreCase(personId)) {
            ...
            if (travelPurposeArray[0].equalsIgnoreCase("PER")) {
                ...
                travelCode.setValue(" ");
                AdfFacesContext.getCurrentInstance().addPartialTarget(travelCode);
                valueChangeEvent.getComponent().processUpdates(FacesContext.getCurrentInstance());
            JSFUtils.addFacesErrorMessage("Please select Employee Name");
            }
        AdfFacesContext.getCurrentInstance().addPartialTarget(lineTable);
    }
    ...
    public void onClickSubmit(ActionEvent actionEvent) {
        ...
        if (!result) {
            ...
            System.out.println("Value ---- " + introHdrRow.getAttribute("Consent").toString());
            if (introHdrRow.getAttribute("Consent").toString() != null &&
                !"Y".equalsIgnoreCase(introHdrRow.getAttribute("Consent").toString())) {
                JSFUtils.addFacesErrorMessage("Consent to keep the personal data is not accepted. Please accept by clicking the check box");
            } else {
                String xmlData = null;
                String[] a = null;
                ...
                String p_EmployeeNumber =
                    ...
                    HdrVo.getCurrentRow()
                                                                                                                 ...
                                                                                                                 .toString();
                String p_travelReq =
                    HdrVo.getCurrentRow().getAttribute("TravelRequestNo") == null ? " " :
                    HdrVo.getCurrentRow()
                ...
                String p_UserName = (String) ADFContext.getCurrent()
                                                       .getSessionScope()
                                                       .get("userName");
                
                String pCreatedBy = HdrVo.getCurrentRow().getAttribute("CreatedBy") == null ? " " : HdrVo.getCurrentRow()
                                                                                                                     .getAttribute("CreatedBy")
                                                                                                                     .toString();
                               
                String pLastUpdateBy = HdrVo.getCurrentRow().getAttribute("LastUpdatedBy") == null ? " " : HdrVo.getCurrentRow()
                                                                                                                     .getAttribute("LastUpdatedBy")
                                                                                                                     .toString();
                String pLastUpdateLogin = HdrVo.getCurrentRow().getAttribute("LastUpdateLogin") == null ? " " : HdrVo.getCurrentRow()
                                                                                                                     .getAttribute("LastUpdateLogin")
                                                                                                                     .toString();
                ...
                while (rs.hasNext()) {
                    Row r = rs.next();
                    p_Relationship.add(r.getAttribute("Relation") == null ? " " :
                                       r.getAttribute("Relation").toString());
                    ...
                    p_PassportNo.add(r.getAttribute("PassportNo") == null ? " " :
                                     r.getAttribute("PassportNo").toString());
                ...
                System.err.println("p_UserName" + p_UserName);
                
                System.err.println("------------pCreatedBy-----------------" + pCreatedBy);
                System.err.println("-------------pLastUpdateBy-----------------" + pLastUpdateBy);
                System.err.println("--------------pLastUpdateLogin----------------" + pLastUpdateLogin);
                
                
                
                xmlData =
                    ApprovalPayload.businessTypeXMLData(p_EmployeeName, p_EmployeeNumber, p_travelReq, p_Country,
                                                        p_Email, p_travelPurposeCode, p_BusinessGroup, p_Remarks,p_UserName,
                                                        p_Relationship, p_FullName, p_DateOfBirth, p_PlaceOfBirth,
                                                        p_PassportNo, p_PassportIssueDate, p_PassportExpiryDate,
                                                        p_ProposalTravelDate, p_ExpectedTravelDate);
                
                
                System.err.println("xmlData--------------------------------------- =>" + xmlData);
                
                System.err.println("before payload--------------------------------------- =>");
                a = ApprovalProcess.invokeWsdl(xmlData, travelReqWSDL, ApprovalPayload.TRAVEL_REQ_METHOD);
                
                System.err.println("after payload--------------------------------------- =>");
                
                if (a[0] != null && a[0].equals("True")) {
                        if (introHdrRow != null) {
                                           
                                           System.err.println("----------------TEST ELSE----------------");
                                           
                                           introHdrRow.setCreatedBy(pCreatedBy);
                                           introHdrRow.setLastUpdatedBy(pLastUpdateBy);
                                           introHdrRow.setLastUpdateLogin(pLastUpdateLogin);
                                        
                              
                                            System.out.println("CreatedBy  "+introHdrRow.getCreatedBy());
                                            System.out.println("LastUpdatedBy  "+introHdrRow.getLastUpdatedBy());
                                            System.out.println("LastUpdateLogin  "+introHdrRow.getLastUpdateLogin());
    ...
    public void onChangeConsentValue(ValueChangeEvent valueChangeEvent) {
        ...
        IntroLtrHdr_VORowImpl introHdrRow = (IntroLtrHdr_VORowImpl) introHdrIter.getCurrentRow();
        if (introHdrRow != null) {
            introHdrRow.setConsent(newValue);
        }
        AdfFacesContext.getCurrentInstance().addPartialTarget(consent);
    }

    public void setConsent(RichSelectBooleanCheckbox consent) {
        this.consent = consent;
    }

    public RichSelectBooleanCheckbox getConsent() {
        return consent;
    }

    public void setTravelCode(RichSelectOneChoice travelCode) {
        this.travelCode = travelCode;
    }

    public RichSelectOneChoice getTravelCode() {
        return travelCode;
    }
    ...
    public String onClickSave() {
            ...
            ViewObject HdrVO = com.view
                                          .utils
                                          .ADFUtils
                                          .findIterator("IntroLtrHdr_VOIterator")
                                          .getViewObject();
        IntroLtrHdr_VORowImpl hdrRow = (IntroLtrHdr_VORowImpl) HdrVO.getCurrentRow();
                   hdrRow.setApprovedStatus("DRAFT");
                    HdrVO.applyViewCriteria(null);
       
      
      
        String userName ;      
        userName = (String) ADFContext.getCurrent() .getSessionScope().get("userName");
        
        System.out.println( "username   " +userName);    
    
         hdrRow.setCreatedBy(userName);
         hdrRow.setLastUpdatedBy(userName);
      

        System.out.println( "CreatedBy   " + hdrRow.getAttribute("CreatedBy"));
        System.out.println( "LastUpdatedBy    " + hdrRow.getAttribute("LastUpdatedBy"));
